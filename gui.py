...
def check_by_claim():
    ...
    if matched:
        ...
        write("\n📌 Best related matches (Doesn't confirm your claim directly):", "white")
        for art in articles[:3]:
            match_s = next(s for a, s in scored_articles if a == art)
            write(f"   • {art['source']} ({match_s:.0f}%) — {art['title'][:55]}...", "cyan")
...
count_spin = ttk.Spinbox(count_frame, from_=1, to=100,

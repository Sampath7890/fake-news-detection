...
with tab1:
    ...
    with col_b:
        count = st.slider("Articles", 1, 10, 5, label_visibility="collapsed")
        ...
        if not claim.strip():
            ...
            if not articles:
                ...
                for article in articles:
                    ...
                    score = len(claim_words & words) / len(claim_words) * 100 \
                            if claim_words else 0
                    if score >= 40:
                ...
                if matched:
                    ...
                    st.markdown('<div class="section-label" style="margin-top:1rem;">Related Articles</div>',
                                unsafe_allow_html=True)
                    for art in articles[:3]:
                        st.markdown(f"""
                        <div class="card" style="padding:1rem 1.2rem;">
                            <div class="card-meta">{art['source']}</div>
                            <div style="font-size:0.9rem;color:#e8e8f0;">{art['title'][:80]}…</div>
                            <a href="{art['url']}" target="_blank"
                               style="font-size:0.78rem;color:#63b3ed;text-decoration:none;">
                                ↗ Read
